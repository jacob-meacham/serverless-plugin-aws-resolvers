...
async function getECSValue(key, awsParameters) {
  ...
  const ecs = new _awsSdk.default.ElastiCache({ ...awsParameters,
    apiVersion: '2015-02-02'
  });
  const result = await ecs.describeCacheClusters({
    CacheClusterId: key,
...
async function getESSValue(key, awsParameters) {
  ...
  const ess = new _awsSdk.default.ES({ ...awsParameters,
    apiVersion: '2015-01-01'
  });
  const result = await ess.describeElasticsearchDomain({
    DomainName: key
  }).promise();
...
async function getEC2Value(key, awsParameters) {
  const ec2 = new _awsSdk.default.EC2({ ...awsParameters,
    apiVersion: '2015-01-01'
  });
...
async function getKinesisValue(key, awsParameters) {
  ...
  const kinesis = new _awsSdk.default.Kinesis({ ...awsParameters,
    apiVersion: '2013-12-02'
  });
  const result = await kinesis.describeStream({
    StreamName: key
  }).promise();
...
async function getDynamoDbValue(key, awsParameters) {
  ...
  const dynamodb = new _awsSdk.default.DynamoDB({ ...awsParameters,
...
async function getRDSValue(key, awsParameters) {
  ...
  const rds = new _awsSdk.default.RDS({ ...awsParameters,
...
async function getRDSAuroraValue(key, awsParameters) {
  _winston.default.debug(`Resolving RDS Aurora cluster with name ${key}`);
...
async function getCFPhysicalResourceId(key, awsParameters) {
  ...
  const cf = new _awsSdk.default.CloudFormation({ ...awsParameters,
    apiVersion: '2014-10-31'
  });
  ...
  const result = await cf.describeStackResource({
...
async function getAPIGatewayValue(key, awsParameters) {
  ...
  const apigateway = new _awsSdk.default.APIGateway({ ...awsParameters,
...
async function getAPIGatewayV2Value(key, awsParameters) {
  ...
  const apigateway = new _awsSdk.default.ApiGatewayV2({ ...awsParameters,
